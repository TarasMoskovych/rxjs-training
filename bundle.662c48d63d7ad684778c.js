...
function fromEvent(target, eventName, options, resultSelector) {
    ...
    if (resultSelector) {
        return fromEvent(target, eventName, options).pipe(map_map(function (args) { return isArray(args) ? resultSelector.apply(void 0, args) : resultSelector(args); }));
    }
...
var mapResponse = /*@__PURE__*/ map_map(function (x, index) { return x.response; });
...
function scan_scan(accumulator, seed) {
    var hasSeed = false;
...
function reduce(accumulator, seed) {
    if (arguments.length >= 2) {
        return function reduceOperatorFunctionWithSeed(source) {
            return pipe_pipe(scan_scan(accumulator, seed), takeLast(1), defaultIfEmpty(seed))(source);
        };
    }
    return function reduceOperatorFunction(source) {
        return pipe_pipe(scan_scan(function (acc, value, index) { return accumulator(acc, value, index + 1); }), takeLast(1))(source);
    };
}
...
function exhaustMap(project, resultSelector) {
    if (resultSelector) {
        return function (source) { return source.pipe(exhaustMap(function (a, i) { return from(project(a, i)).pipe(map_map(function (b, ii) { return resultSelector(a, b, i, ii); })); })); };
    }
    return function (source) {
...
var animationFrame = (/* unused pure expression or super */ null && (animationFrameScheduler));
...
function switchMap(project, resultSelector) {
    if (typeof resultSelector === 'function') {
        return function (source) { return source.pipe(switchMap(function (a, i) { return from(project(a, i)).pipe(map_map(function (b, ii) { return resultSelector(a, b, i, ii); })); })); };
    }
...
;// CONCATENATED MODULE: ./src/ts/libs/helpers.ts



var svgNamespace = 'http://www.w3.org/2000/svg';
var usersUrl = 'https://jsonplaceholder.typicode.com/users';
var sampleData$ = ajax.getJSON(usersUrl).pipe(switchMap(function (users) { return of.apply(void 0, users); }));
var add = {
    li: printLi,
    div: printDiv,
    line: printLine,
...
var getRandomBackground = function () { return "background-color: rgb(" + Math.random() * 255 + ", " + Math.random() * 255 + ", " + Math.random() * 255 + ") !important"; };
var clock = {
    seconds: 60,
    hours: 12,
    radius: 125,
...
var Clock = /** @class */ (function () {
    function Clock(parent) {
        ...
        svg.id = 'display-svg';
        var bg = document.createElementNS(svgNamespace, 'circle');
        var secondsLine = document.createElementNS(svgNamespace, 'line');
        var minutesLine = document.createElementNS(svgNamespace, 'line');
        var hoursLine = document.createElementNS(svgNamespace, 'line');
        var pin = document.createElementNS(svgNamespace, 'circle');
...
function tickForward(n) {
    return pipe(take(n), map(function () {
        return clock.increments(60);
    }), scan(function (acc, increments) {
        var newAngle = acc + increments;
...
function moveTime(n, type) {
    return map_map(function (t) {
        var m = t[type] * clock.increments(n);
        var b = m - clock.increments(n) * (n * 0.25);
        return b;
    });
}
var counter = interval(1000).pipe(take_take(20), map_map(function (x) {
    return "Count of: " + (x + 1);
}));
var delayByThree = new Promise(function (resolve) {
    setTimeout(function () {
...
function printDiv(value) {
    var main = document.getElementById('image-container');
...
function updateLine(element, angle, type) {
    ...
    var newX2 = width / 2 + Math.cos(angle) * radius;
    var newY2 = height / 2 + Math.sin(angle) * radius;
...
function buildArray(n) {
    return new Array(n).fill(1).map(function (_, i) {
        return i + 1;
    });
}
function elapsed() {
    var start = animationFrameScheduler.now();
    return interval(0, animationFrameScheduler).pipe(map_map(function () { return animationFrameScheduler.now() - start; }));
}
function animate(durationTime) {
    return elapsed().pipe(map_map(function (elapsedTime) {
...
;// CONCATENATED MODULE: ./src/ts/libs/observable.ts
var CustomObservable = /** @class */ (function () {
    function CustomObservable(observe) {
        this.observe = observe;
        this.completed = false;
    }
    CustomObservable.prototype.subscribe = function (next, error, complete) {
        var _this = this;
        var observer = {
            next: function (value) { return !_this.completed && next && next(value); },
            error: function (err) { return !_this.completed && error && error(err); },
            complete: function () {
                if (complete) {
        ...
        var teardown = this.observe(observer);
        return {
...
;// CONCATENATED MODULE: ./src/ts/libs/index.ts
...
;// CONCATENATED MODULE: ./src/ts/components/clock.ts



var initClock = function () {
    var clock = new Clock('chart');
    ...
    range(1, 60).subscribe(function (tickMark) { return add.line(tickMark, 'seconds'); });
    range(1, 12).subscribe(function (tickMark) { return add.line(tickMark, 'hours'); });
    var timeTick$ = interval(1000).pipe(map_map(function () {
        var time = new Date();
        return {
            hours: time.getHours(),
    ...
    timeTick$.pipe(moveTime(60, 'minutes')).subscribe(function (angle) {
        update.line(minuteHand, angle, 'minute');
    });
    timeTick$.pipe(moveTime(60, 'seconds')).subscribe(function (angle) {
        update.line(secondHand, angle, 'second');
    });
    timeTick$.pipe(moveTime(12, 'hours')).subscribe(function (angle) {
...
;// CONCATENATED MODULE: ./src/ts/components/drawer.ts


var initDrawer = function () {
    var canvas = document.querySelector('.canvas-draw');
    var context = canvas.getContext('2d');
    var brush = function (coords) {
        context.lineWidth = 5;
        context.lineTo(coords.x, coords.y);
        context.stroke();
    };
    var moves$ = fromEvent(canvas, 'mousemove');
    var down$ = fromEvent(canvas, 'mousedown');
    var up$ = fromEvent(canvas, 'mouseup');
    down$
        .pipe(tap(function (evt) {
        context.strokeStyle = 'purple';
        context.beginPath();
        context.moveTo(evt.offsetX, evt.offsetY);
    }), switchMap(function () { return moves$.pipe(map_map(function (evt) { return ({ x: evt.offsetX, y: evt.offsetY }); }), takeUntil(up$)); }))
        .subscribe(brush);
};
...
function now() {
  return clockNow || (setFrame(clearNow), clockNow = timer_clock.now() + clockSkew);
}
...
function wake() {
  clockNow = (clockLast = timer_clock.now()) + clockSkew;
...
function poke() {
  var now = timer_clock.now(), delay = now - clockLast;
...
function sleep(time) {
  ...
  if (delay > 24) {
    if (time < Infinity) timeout = setTimeout(wake, time - timer_clock.now() - clockSkew);
    if (timer_interval) timer_interval = clearInterval(timer_interval);
  } else {
    if (!timer_interval) clockLast = timer_clock.now(), timer_interval = setInterval(poke, pokeDelay);
...
;// CONCATENATED MODULE: ./src/ts/components/visualizer.ts


var initVisualizer = function () {
    var waveData = new Subject();
    ...
    waveData.subscribe(function (d) { return renderFullLine(d); });
    interval(0, animationFrameScheduler).subscribe(renderLoop);
};

;// CONCATENATED MODULE: ./src/ts/components/index.ts
...
function getUser() {
    ajax.getJSON(usersUrl + "/1")
        .subscribe(function (user) { return add.li(JSON.stringify(user)); });
    // fromFetch(`${usersUrl}/1`)
    //   .pipe(switchMap((response: Response) => response.json()));
}
interval(2000).subscribe(function () {
    document
...
fromEvent(document.querySelector('input'), 'keyup')
    ...
    .subscribe(function (e) { return document.querySelector('.brand-logo').textContent = e.target.value; });
////////////////////////////////////
interval(100)
    .pipe(take_take(10), 
...
reduce(function (acc, value) {
    var n = value + 1;
    return __spreadArray(__spreadArray([], acc, true), [acc[n] + acc[n - 1]], false);
}, [0, 1]))
...
fromEvent(document.querySelector('button'), 'click')
    .pipe(exhaustMap(function () { return animate(2000); }))
    .subscribe(function (t) { return circle.style.left = "calc(" + t * 100 + "% - 50px)"; });
////////////////////////////////////
// cold (unicast)
var obs$ = new Observable(function (observer) { return observer.next(Date.now()); });
obs$.subscribe(add.li);
obs$.subscribe(add.li);
// hot (multicast)
var sub = new Subject();
sub.subscribe(function (val) { return add.li('S: ' + val); });
sub.subscribe(function (val) { return add.li('S: ' + val); });
sub.next(Date.now());
